...
echo '<h2>Izmjeni korisnika</h2>';

$zapis = mysqli_query($con,"SELECT * FROM User WHERE idUser=".$idUser);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniUser.php">';
echo '<div class="form_settings">';
echo '<p>Korisničko ime:</p> <input type="text" name="imeUser" value="'.$row['imeUser'].'"><br>';
echo '<p>Lozinka:</p> <input type="text" name="lozinkaUser" value="'.$row['lozinkaUser'].'"><br>';
echo '<input type="hidden" value="'.$row['idUser'].'" name="idUser"/>
<input class="submit" type="submit" value="Izmjeni korisnika" />
</div>
</form>
<br>';
}
...
echo "<a href='http://iws.mev.hr/mrisek/adminUser.php'>Povratak na prethodnu stranicu</a>";
