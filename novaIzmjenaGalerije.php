...
echo '<h2>Izmjeni sliku</h2>';

$zapis = mysqli_query($con,"SELECT * FROM Galerija WHERE idGalerija=".$idGalerija);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniGaleriju.php">';
echo '<div class="form_settings">';
echo '<p>Ime galerije:</p> <input type="text" name="nazivGalerija" value="'.$row['nazivGalerija'].'"><br>';
echo '<p>Naziv slike:</p> <input type="text" name="slikaGalerija" >(*.jpg, *.gif)<br>';
echo '<input type="hidden" value="'.$row['idGalerija'].'" name="idGalerija"/>
<input class="submit" type="submit" value="Izmjeni sliku" />
</div>
</form>
<br>';
}
...
echo "<a href='http://iws.mev.hr/mrisek/adminGalerija.php'>Povratak na prethodnu stranicu</a>";
