...
$idNavigacija = mysqli_real_escape_string($con, $_POST['idNavigacija']);

mysql_query("SET NAMES 'utf8'");
mysql_query("SET CHARACTER_SET 'utf8_unicode_ci'");

echo '<h2>Izmjeni navigaciju</h2>';

$zapis = mysqli_query($con,"SELECT * FROM Navigacija WHERE idNavigacija=".$idNavigacija);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniNavigaciju.php">';
echo '<div class="form_settings">';
echo '<p>Naziv:</p> <input type="text" name="nazivNavigacija" value="'.$row['nazivNavigacija'].'"><br>';
echo '<p>Link:</p> <input type="text" name="linkNavigacija" value="'.$row['linkNavigacija'].'"><br>';
echo '<p>Opis:</p> <textarea rows="3" name="opisNavigacija" cols="20">'.$row['opisNavigacija'].'</textarea><br>';
echo '<input type="hidden" value="'.$row['idNavigacija'].'" name="idNavigacija"/>
<input class="submit" type="submit" value="Izmjeni navigaciju" />
</div>
</form>
<br>';
}
...
echo "<a href='http://iws.mev.hr/mrisek/adminNavigacija.php'>Povratak na prethodnu stranicu</a>";
