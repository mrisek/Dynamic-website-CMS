...
$idLink = mysqli_real_escape_string($con, $_POST['idLink']);

mysql_query("SET NAMES 'utf8'");
mysql_query("SET CHARACTER_SET 'utf8_unicode_ci'");

echo '<h2>Izmjeni link</h2>';

$zapis = mysqli_query($con,"SELECT * FROM Link WHERE idLink=".$idLink);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniLink.php">';
echo '<div class="form_settings">';
echo '<p>Naziv:</p> <input type="text" name="nazivLink" value="'.$row['nazivLink'].'"><br>';
echo '<p>Sadržaj:</p> <textarea rows="4" name="sadrzajLink" cols="20">'.$row['sadrzajLink'].'</textarea><br>';
echo '<input type="hidden" value="'.$row['idLink'].'" name="idLink"/>
<input class="submit" type="submit" value="Izmjeni link" />
</div>
</form>
<br>';
}
...
echo "<a href='http://iws.mev.hr/mrisek/adminLink.php'>Povratak na prethodnu stranicu</a>";
