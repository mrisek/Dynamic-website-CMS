...
echo '<h2>Izmjeni članak</h2>';

$zapis = mysqli_query($con,"SELECT * FROM Clanak WHERE idClanak=".$idClanak);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniClanak.php">';
echo '<div class="form_settings">';
echo '<p>Naslov:</p> <input type="text" name="naslovClanak" value="'.$row['naslovClanak'].'"><br>';
echo '<p>Datum:</p> <input type="text" name="datumClanak" value="'.date("Y-m-d").'"><br>';
echo '<p>Sadržaj članka:</p> <textarea rows="15" name="sadrzajClanak" cols="20">'.$row['sadrzajClanak'].'</textarea><br>';
echo '<input type="hidden" value="'.$row['idClanak'].'" name="idClanak"/>
<input class="submit" type="submit" value="Izmjeni članak" />
</div>
</form>
<br>';
}
