...
$idKontakt = mysqli_real_escape_string($con, $_POST['idKontakt']);

mysql_query("SET NAMES 'utf8'");
mysql_query("SET CHARACTER_SET 'utf8_unicode_ci'");

echo '<h2>Izmjeni Kontakt</h2>';

$zapis = mysqli_query($con,"SELECT * FROM Kontakt WHERE idKontakt=".$idKontakt);
while($row = mysqli_fetch_array($zapis))
{
echo '<form method="post" action="izmjeniKontakt.php">';
echo '<div class="form_settings">';
echo '<p>Ime:</p> <input type="text" name="imeKontakt" value="'.$row['imeKontakt'].'"><br>';
echo '<p>Prezime:</p> <input type="text" name="prezimeKontakt" value="'.$row['prezimeKontakt'].'"><br>';
echo '<p>E-mail:</p> <input type="text" name="emailKontakt" value="'.$row['emailKontakt'].'"><br>';
echo '<p>Poruka:</p> <textarea rows="8" name="porukaKontakt" cols="20">'.$row['porukaKontakt'].'</textarea><br>';
echo '<input type="hidden" value="'.$row['idKontakt'].'" name="idKontakt"/>
<input class="submit" type="submit" value="Izmjeni kontakt" />
</div>
</form>
<br>';
}
...
echo "<a href='http://iws.mev.hr/mrisek/adminKontakt.php'>Povratak na prethodnu stranicu</a>";
