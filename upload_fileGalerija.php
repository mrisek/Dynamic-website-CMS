...
&& in_array($extension, $allowedExts)) {
  if ($_FILES["file"]["error"] > 0) {
	...
	echo "Uspješno ste dodali novu sliku u galeriju!<br><br>";
...
$idGalerija = mysqli_real_escape_string($con, $_POST['idGalerija']);
$nazivGalerija = mysqli_real_escape_string($con, $_POST['nazivGalerija']);
$slikaGalerijaNaziv = mysqli_real_escape_string($con, $_POST['slikaGalerija']);

$slikaGalerija = '<span class="left"><img src="galerija/'. $_FILES["file"]["name"].'" alt="example graphic" /></span>';

mysql_query("SET NAMES 'utf8'");
mysql_query("SET CHARACTER_SET 'utf8_unicode_ci'");

$sql="INSERT INTO Galerija (idGalerija, nazivGalerija, slikaGalerija)
VALUES ('$idGalerija', '$nazivGalerija', '$slikaGalerija')";

if (!mysqli_query($con,$sql)) {
  die('Error: ' . mysqli_error($con));
}
	
    if (file_exists("galerija/" . $_FILES["file"]["name"])) {
	  ...
	  echo "<br><br><a href='http://iws.mev.hr/mrisek/adminGalerija.php'>Povratak na prethodnu stranicu</a>";
    } else {
      move_uploaded_file($_FILES["file"]["tmp_name"],
      "galerija/" . $_FILES["file"]["name"]);
      echo "Pohranjeno u mapu: " . "galerija/" . $_FILES["file"]["name"];
	  echo "<br><br><a href='http://iws.mev.hr/mrisek/adminGalerija.php'>Povratak na prethodnu stranicu</a>";
    }
  }
} else {
  echo "Morate odabrati sliku tipa *.JPG ili *.GIF veličine do 40 kB";
  echo "<br><br><a href='http://iws.mev.hr/mrisek/adminGalerija.php'>Povratak na prethodnu stranicu</a>";
}
?>
